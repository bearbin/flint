...
contract FlightManager {
  ...
  constructor(bytes32 _flightID, address _admin, uint256 _ticketPrice, uint256 _numSeats) public {
    assembly {
      ...
      function init() {
        ...
        sstore(add(0, 5), 0)
        sstore(add(0, 7), 0)
        FlightInformation_init_String(add(0, 0), 0, _flightID)
        sstore(add(0, 2), _admin)
        sstore(add(0, 3), _ticketPrice)
        sstore(add(0, 4), _numSeats)
      }
      ...
      function FlightInformation_init_String(_flintSelf, _flintSelf$isMem, _flightID)  {
      ...
      function FlightInformation_cancelFlight(_flintSelf, _flintSelf$isMem)  {
        flint$store(flint$computeOffset(_flintSelf, 1, _flintSelf$isMem), 1, _flintSelf$isMem)
      }
  ...
  function () public payable {
    assembly {
      ...
      case 0xa3ddd9b1 /* getFlightID() */ {
        
        flint$return32Bytes(getFlightID())
      }
      
      case 0x5fc6767a /* isFlightCancelled() */ {
        
        flint$return32Bytes(isFlightCancelled())
      }
      
      case 0x87bb7ae0 /* getTicketPrice() */ {
        
        flint$return32Bytes(getTicketPrice())
      }
      ...
      case 0x09aa69c2 /* cancelFlight() */ {
        ...
        _flintCallerCheck := add(_flintCallerCheck, flint$isValidCallerCapability(sload(2)))
      ...
      case 0x54ddd5d6 /* retrieveRefund() */ {
        let _flintCallerCheck := 0
        _flintCallerCheck := add(_flintCallerCheck, flint$isCallerCapabilityInArray(5))
        if eq(_flintCallerCheck, 0) { revert(0, 0) }
      ...
      function buy()  {
        ...
        Flint$Global_assert_Bool(eq(_amountGiven, sload(add(0, 3))))
        Flint$Global_assert_Bool(gt(sload(add(0, 4)), 0))
        Flint$Global_assert_Bool(eq(sload(add(0, 1)), 0))
        Wei_transfer_$inoutWei(flint$storageDictionaryOffsetForKey(7, _caller), 0, _value, 1)
        sstore(flint$storageArrayOffset(5, sload(add(0, 6))), _caller)
        sstore(add(0, 6), flint$add(sload(add(0, 6)), 1))
        sstore(add(0, 4), flint$sub(sload(add(0, 4)), 1))
      }
      
      function getFlightID() -> ret {
        let _caller := caller()
        ret := sload(add(0, 0))
      }
      
      function isFlightCancelled() -> ret {
        let _caller := caller()
        ret := sload(add(0, 1))
      }
      
      function getTicketPrice() -> ret {
        let _caller := caller()
        ret := sload(add(0, 3))
      }
      ...
      function retrieveRefund()  {
        ...
        Flint$Global_send_Address_$inoutWei(_passenger, flint$storageDictionaryOffsetForKey(7, _passenger), 0)
      }
...
interface _InterfaceFlightManager {
  ...
  function getFlightID() view external returns (bytes32 ret);
  function isFlightCancelled() view external returns (uint256 ret);
  function getTicketPrice() view external returns (uint256 ret);
  function getNumRemainingSeats() view external returns (uint256 ret);
  function cancelFlight() external;
  function retrieveRefund() view external;
